...
# _sshcfg_store_validate_* "input_value"
# => Returns 0 if valid
# => Prints error message and returns 1 if invalid
#
# Examples:
# _sshcfg_store_validate_alias "server1"     => 0 (valid)
# _sshcfg_store_validate_alias "server 1"    => "Alias cannot contain spaces" + return 1
# _sshcfg_store_validate_hostname ".bad"     => "Hostname cannot start with a dot" + return 1
# _sshcfg_store_validate_port "99999"        => "Port must be less than 65536" + return 1
...
_sshcfg_store_validate_alias() {
    ...
    [[ "$alias" =~ ^[a-zA-Z] ]] || \
        { echo 'Alias must start with a letter'; exit_code=1; }
    [[ -n $(_sshcfg_store_alias_list | grep -Fx "$alias") ]] && \
